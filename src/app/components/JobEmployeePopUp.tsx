...
const JobEmployeePopUp: React.FC<Props> = ({ job, applications, onClose, onUpdateStatus }) => {

  // Function to handle changing the status of an application to "Sent"
  const handleChangeStatus = (applicationId: string) => {
          ...
          {applications.length > 0 ? (
            applications.map((application) => (
              <div key={application._id} className="flex justify-between items-center">
                ...
                <button
                  onClick={() => handleChangeStatus(application._id)}
                  className="bg-blue-500 text-white px-4 py-2 rounded disabled:bg-gray-400"
                  disabled={application.status === 'Sent'}
                >
                  {application.status === 'Sent' ? 'Already Sent' : 'Mark as Sent'}
                </button>
              </div>
            ))
          ) : (
